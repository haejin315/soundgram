...
const myNfts = {
  ...
  action: async function (nfckey_id, album_id, token_id, action_type) {
    ...
    var buyerId=await this.getTokenBuyer(token_id);
    var buyerAddr=await this.getAddrToId(buyerId);
    console.log("action",buyerAddr);
 //TODO 지금은 데이터베이스내부에서만 거래가 이루어지고 있는 상태
           //tsContract not define 에러 
    this.sellAlbumToBuyer(buyerAddr,token_id);
  ...
  getTokenBuyer: async function(token_id){
    console.log("token id", token_id);
    $.ajax({
      url: "./php/get_token_buyer.php",
      data: { 
        "token_id": token_id,
      },
      type: "GET",
      dataType: "JSON",
      success: function(data) {
        console.log("getTokenBuyer success!");
        console.log("getTokenBuyer!");
        console.log(data["TokenBuyer"]);
        alert(data["TokenBuyer"]);
        return data["TokenBuyer"]
      },
      error:function(request,status,error){
        alert(error);
      }
  ...
  getAddrToId: async function(user_id){
    ...
    $.ajax({
      ...
      dataType: "JSON",
      success: function(data) {
        console.log("getAddrToId success!");
        console.log("getAddrToId!");
  ...
  getTokenOwner: async function(token_id){
    $.ajax({
      url: "./php/get_token_owner.php",
      data: { 
        "token_id": token_id,
      },
      type: "GET",
      dataType: "JSON",
      success: function(data) {
        console.log("getTokenOwner second");
        console.log(data["TokenOwner"]);
  ...
  sellAlbumToBuyer: async function(buyerAddr, tokenId) {
        console.log("sellAlbumToBuyer start");
    try {
      ...
      const { rawTransaction: senderRawTransaction } = await cav.klay.accounts.signTransaction({
        type: 'FEE_DELEGATED_SMART_CONTRACT_EXECUTION',
        //from: buyerAddr,
        from: '0xdef8f7f320427302cfeafe306f68e4c908af4956',
        to:   DEPLOYED_ADDRESS_ALBUMSALES,
        data: salesContract.methods.purchaseToken(tokenId).encodeABI(),
        gas:  '500000',
        value: price,
      },'0xe95d5d74cfab8ccd8f71712221e035e1c8a1048013b6f77847f20c80c1ac3de5')
      ...
      .then(function(receipt){
        if (receipt.transactionHash) {         
          console.log("sellAlbumToBuyer sucesse");
          alert(receipt.transactionHash);
        }
      });
    } catch (err) {
